...
function render(date){selectedDate=date;const data=(byDate[date]||[]).slice();data.sort((a,b)=>{const av=a[sortKey],bv=b[sortKey];const cmp=typeof av==='number'?av-bv:String(av||'').localeCompare(String(bv||''),'ko');return cmp*sortDirection;});const avg=data.length?data.reduce((sum,r)=>sum+r.rs,0)/data.length:0;document.getElementById('stockCount').textContent=data.length+'개';document.getElementById('avgRs').textContent=data.length?avg.toFixed(2):'-';document.getElementById('etfCount').textContent=data.filter(r=>r.etfGood===PASS).length+'개';document.getElementById('updated').textContent=date?date+' 장 마감 기준':'';renderIndices(date);const body=document.getElementById('tbody');body.innerHTML=data.map((r,i)=>'<tr><td class="rank">'+(i+1)+'</td><td><span class="name">'+esc(r.name)+'</span><span class="code">'+esc(r.code)+'</span></td><td>'+marketCap(r.marketCap)+'</td><td class="positive">'+Number(r.rs).toFixed(2)+'</td><td class="'+tone(r.ma20Gap)+'">'+signed(r.ma20Gap)+'</td><td><span class="pill '+(r.volatilityContraction==='O'?'pass':'fail')+'">'+esc(r.volatilityContraction)+'</span><div class="change '+tone(r.priceChange5d)+'">'+signed(r.priceChange5d)+'</div></td><td><strong>'+esc(r.industry)+'</strong></td><td><div class="theme">'+esc(r.theme)+'</div></td><td><span class="pill '+(r.etfGood===PASS?'pass':'fail')+'">'+esc(r.etfGood||'-')+'</span><div class="etf">'+esc(r.etfNames||'-')+'</div></td></tr>').join('');document.getElementById('empty').hidden=data.length>0;renderCalendar();}
function renderCalendar(){const y=shownMonth.getFullYear(),m=shownMonth.getMonth(),first=new Date(y,m,1).getDay(),last=new Date(y,m+1,0).getDate(),cells=[];document.getElementById('monthTitle').textContent=y+'년 '+(m+1)+'월';for(let i=0;i<first;i++)cells.push('<div class="day blank"></div>');for(let day=1;day<=last;day++){const date=iso(y,m,day),data=byDate[date]||[],priority=data.filter(r=>r.volatilityContraction==='O');let inside='<span class="date">'+day+'</span>';if(data.length){inside+='<span class="countchip">'+data.length+'종목</span>';if(priority.length){inside+='<span class="prioritytitle">우선확인종목</span>'+priority.map(r=>'<span class="stockchip">'+esc(r.name)+'</span>').join('');}}const week=(first+day-1)%7;cells.push('<button class="day '+(data.length?'hasdata ':'')+(date===selectedDate?'selected ':'')+(week===0?'sunday':week===6?'saturday':'')+'" '+(data.length?'data-date="'+date+'"':'disabled')+'>'+inside+'</button>');}while(cells.length%7)cells.push('<div class="day blank"></div>');document.getElementById('calendarDays').innerHTML=cells.join('');}
